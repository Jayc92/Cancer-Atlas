...
DECLARED = [
    ...
    {'key': 'js/organs/brain.js|comment|MENINGIOMA — every citation verified dir', 'reason': 'TRUE — the same "every citation verified directly at the source" header convention used at the top of every entity\'s own comment block throughout this atlas (e.g. skin.js\'s BCC/SCC/MCC headers), restating this pass\'s own verification discipline rather than asserting a new, checkable fact about the corpus', 'until': None},
    {'key': 'js/organs/brain.js|note|essentially molecularly exclusive of eve', 'reason': 'TRUE and self-evidencing — quotes the exact gene list directly from its own source (Sievers et al., 2025: "particularly TRAF7, KLF4, SMARCE1, AKT1, SMO, SUFU, PTCH1, YAP1, or POLR2A variants") rather than asserting an unscoped guess; flagged only because the record sits on the TTN passenger entry rather than on BAP1 itself, a scope mismatch between record and claim subject, not a false or unscoped claim', 'until': None},
    {'key': 'js/organs/cervix.js|comment|since no other organ in this atlas is ce', 'reason': 'TRUE, confirmed by census: grep for a bare "cervical" alias across every js/organs/*.js organEntry finds only this one file — no collision to check', 'until': None},
    {'key': 'js/organs/cervix.js|comment|Isolated via this atlas\'s established Bl', 'reason': 'TRUE — the same import/identify-sub-meshes/delete-rest/unparent-CLEAR_KEEP_TRANSFORM/recenter-by-bbox/export sequence CLAUDE.md documents for the Colon/Pancreas HRA extractions and for this organ\'s own Uterus source asset, not an invented or unscoped claim about the pipeline\'s history', 'until': None},
    # Lymph Nodes organ authoring pass, 2026-09-14 — four flags, all TRUE and scoped to THIS
    # ORGAN's own eight entities, not the whole atlas, the same self-scoping shape skin.js:596's
    # MCC declaration above already establishes.
    {'key': 'js/histology.js|comment|Lymph Nodes organ (2026-09-14) — eight e', 'reason': 'TRUE — checked directly against every existing histology primitive (drawGlandRing/drawWhorl/drawKeratinPearl/drawCribriformMass/drawFrond/drawSmallCellSheet/drawSingleFileCord) before writing genFL/genMCL/genBL/genCHL/genAITL/genNMZL/genPTCLN; only genCLymph (registered as ndlbcl) reuses verbatim, for the stated reason (identical disease to this atlas\'s own Colon-organ clymph entity)', 'until': None},
    {'key': "js/organs/lymphnodes.js|comment|PROVENANCE, DISCLOSED AS THE SOURCE'S OW", 'reason': 'TRUE — every other HRA-sourced organ mesh in this atlas (kidneys/liver/brain/prostate/breast/pancreas/bladder) is documented in this file\'s own comment history and in CLAUDE.md as Visible Human Dataset-derived; this asset\'s Ce3D mouse-tissue provenance is a real, checked, disclosed exception, not an unscoped guess', 'until': None},
    {'key': 'js/organs/lymphnodes.js|comment|organ-localized disease from every entit', 'reason': 'TRUE and scoped to "every entity modeled here" — this organ\'s own eight cancerEntries, all of which genuinely model nodal/systemic disease, distinct from colon\'s own clymph entity (organ-localized primary colonic lymphoma) as that entity\'s own commit record states explicitly', 'until': None},
    {'key': 'js/organs/lymphnodes.js|desc|Every lymphoma modeled here begins as a ', 'reason': 'TRUE and scoped to "modeled here" — this organ\'s own eight entities (fl/mcl/bl/chl/aitl/ndlbcl/nmzl/ptcln), every one of which is a real lymphoid malignancy arising from a B or T lymphocyte normally resident in lymph-node tissue, per each entity\'s own trunk note', 'until': None},
