...
const DECLARED_ZERO = {
  ...
  cmeso: { reason: 'real disease rarity (under 1% of endocervical adenocarcinoma, this organ\'s rarest entity) — the organ-anchored query itself returns 0 studies under the live filter, and a bare "mesonephric carcinoma" query with no organ anchor also returns zero studies whose own condition strings contain the literal word "mesonephric" anywhere (the 8 studies that query surfaces are unrelated pediatric germ-cell/rhabdoid-tumor trials matched by ClinicalTrials.gov\'s own broad free-text relevance search, not any real "mesonephric" tag)', checked: '2026-09-15' },
};
