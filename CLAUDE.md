...
## FABRICATION IS A DEFECT CLASS DISTINCT FROM DRIFT — NEITHER GATE NOR NEIGHBOURHOOD CAN REACH IT (2026-09-13, user ruling)

**Every citation defect this project has found before this one shares one shape: a real number, used wrong.** Misquoting (a real phrase, altered in transcription), scope drift (a real figure, attached to a broader or narrower population than its source measured), denominator transplants (a real count, paired with the wrong total), certainty drift (a real finding, restated with a stronger or weaker hedge than the source used). Every one of these has a true value sitting somewhere nearby — in the same source, in a neighboring sentence, in the paper's own abstract versus its Results section — and every check this project has built for citations works by comparing a claim against that neighbourhood: does the quoted span occur in the source text, does the fraction match the percentage, does the journal in the record match the journal the identifier resolves to, does the claimed population match what the source's own denominator describes.

**The pcyst EXTENT_STATUS claim was not that.** An earlier draft asserted, in quote marks, that Ziogas et al. 2023 found "more than 90% diagnosed at the localized setting." The word "localized" does not appear anywhere in that paper — not paraphrased, not nearby, not in a different section. And the number itself fails the source's own arithmetic: Ziogas reports clinical N1 (15.7%) and M1 (5.9%) as separate marginals, and by inclusion-exclusion the maximum possible localized (N0M0) share those two marginals could support is 100% − 15.7% = 84.3%, below the claimed >90% regardless of how the two categories overlap. There was no true value nearby to drift from. The claim was not transcribed from something the source said and then altered — it was generated because a favorable-sounding extent contrast was exactly what the surrounding argument wanted, and it read as plausible enough to not trigger a second look.

**This is why fabrication needs to be named as its own class rather than filed under scope drift or certainty drift: the defence is narrower, and knowing which defence applies changes what a verification pass should actually do.** A drift-shaped defect can, in principle, be caught by a check that never reads the source at all — `fraction_check.py` catching a fraction that doesn't match its own stated percentage, `citation_crosscheck.py` catching a journal that disagrees with its own identifier's metadata, `duplicate_figure_check.py` catching two copies of one figure that disagree. Those checks work on the CLAIM'S OWN INTERNAL CONSISTENCY, or on consistency between the claim and a resolvable external record — no one has to open the paper. A fabricated claim has no such internal tell: the quote is well-formed, the number is a plausible-looking percentage, the citation resolves to a real paper on the real topic. **The only thing that catches it is a human (or an agent with no memory of authoring the claim) opening the actual source and finding that the specific sentence, quoted or not, simply is not there.** This is exactly what the independent citation-verification pass (above) is required for — but it is worth stating explicitly that fabrication is the reason the requirement cannot be relaxed to "spot-check the ones that feel uncertain": a fabricated claim, by construction, does not feel uncertain to the person who wrote it, which is the whole danger of it.

**Practical consequence for any future authoring pass:** when a citation-verification agent reports a claim it could not locate in the source, do not default to treating this as a paraphrase-blindness or scope-mismatch problem (data rule 33's own "anchor on the number, not the phrase" method exists for exactly that milder case, and it is the right first move). Check specifically whether the source, read in full, could support the claim AT ALL under any reasonable paraphrase — and if it cannot, and the numbers involved fail the source's own arithmetic besides, record it as fabrication, not drift. The fix is the same either way (correct or remove the claim), but the diagnosis matters for calibrating how much any given verification pass should be trusted going forward: a pass that finds drift is confirming the discipline is working as designed; a pass that finds fabrication is confirming that a claim can look completely unremarkable and still have nothing behind it.

**SECOND CONFIRMED INSTANCE (2026-09-14, Lymph Nodes organ, user ruling): this is now a rate, not an incident, and nothing mechanical reaches it.** The independent citation-verification pass over the new Burkitt lymphoma entry found `js/organs/lymphnodes.js`'s trunk-note header comment quoting Schmitz et al. (*Nature*, 2012, PMID 22885699) as naming "rapidly dividing centroblasts, the putative cell of origin" specifically. The word "centroblast" does not appear anywhere in that paper, checked four independent ways — the paper's own language is "the normal germinal centre B cell." Same shape as the pcyst instance exactly: a well-formed quote, a real citation on the real topic, a claim that read as unremarkable to the person who wrote it (this session's own author, mid-pass, with no more reason to doubt it than the pcyst author had), and no internal inconsistency for any mechanical check to trip on — fixed by dropping the quote marks and presenting the extra specificity as the atlas's own gloss rather than Schmitz's words. **Two fabrications found in two rounds of running this pass is a rate, and a rate changes what the pass IS, not just how carefully it should be read:** every other verification layer in this project's own gate chain (fraction_check, citation_crosscheck, duplicate_figure_check, citation_paren_ledger, pointer_check, absence_claim_check) is a NEIGHBOURHOOD check — it catches a claim disagreeing with itself, with its own metadata, or with an adjacent record. None of the eighteen defects the same pass found in the same round were caught by any of those — the neighbourhood checks all reported clean on this file before and after, because none of the eighteen (fabrication included) has the internal-inconsistency shape those checks are built to find. **The independent citation-verification pass is not an additional layer on top of the gate chain for the fabrication class specifically — for that one class, it is the only layer that exists**, and at a two-for-two hit rate across the only two organs it has been run against as a REQUIRED step (data rule 37), treating it as anything less than required for every future organ/cancer pass would be choosing not to look for the one defect class nothing else in this project can see.

**THIRD CONFIRMED INSTANCE (2026-09-15, Marrow organ, user ruling): three organs, three fabrications, one each — a rate is now a CONSTANT, and the mechanism is worth stating precisely because this instance shows exactly why nothing mechanical reaches it.** The independent citation-verification pass over the new ALL (acute lymphoblastic leukemia) entry found `js/organs/marrow.js`'s BCR-ABL1 trunk note claiming the Philadelphia-positive fraction reaches "up to 31% in one referral-center cohort," attributed to Iacobucci & Mullighan (*J Clin Oncol*, 2017). A direct, exhaustive search of that paper's full text found no occurrence of "31%" anywhere relevant to BCR-ABL1 frequency — the source states only "25% of adult ALL," with no referral-center-specific breakdown and no discussion of frequency rising within the adult age range. No alternative real source for this specific figure could be located either.

**THE FORMULATION THIS INSTANCE EARNS: fabrication produces figures in the right neighbourhood, which is exactly why every mechanical check passes them.** BCR-ABL1-positive (Philadelphia-chromosome-positive) B-ALL is real, and Ph-positive disease genuinely runs around a quarter of adult B-ALL — so "31%" is not an absurd, out-of-range invention; it sits in the correct neighbourhood for a real phenomenon. `fraction_check` sees a well-formed, internally consistent number with nothing nearby to contradict it. `citation_crosscheck` sees a real paper, on the exact right topic, correctly identified by PMID, with author/year/journal all matching. `citation_reach_check` sees a citation that resolves cleanly. **Every mechanical check passes it for the same reason a fabricated number is dangerous in the first place: whoever or whatever produced it had absorbed enough real domain knowledge to place the number close to where the truth actually sits, which is precisely what makes proximity-to-truth useless as a discriminator.** Only reading the source's own sentences, with no stake in the number already being correct, finds that the specific claim is not there.

**Three for three, one fabrication in every organ this pass has been run on, is now a STRUCTURAL FACT about this class of defect, not a current observation that might change with more data.** The independent citation-verification pass is not a additional layer for fabrication specifically — it is the only layer, and every future organ/cancer pass should be planned and budgeted on the assumption that it WILL find at least one, not on the hope that this round is the one that comes back clean. No amount of widening `fraction_check`'s field coverage, adding metadata fields to `citation_crosscheck`, or tightening `duplicate_figure_check`'s comparison window closes this gap, because all three are neighbourhood checks by design and fabrication is defined by having no neighbourhood to check against.
