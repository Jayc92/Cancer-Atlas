...
// THE GROWTH EDGE CHANNEL — rim-blend (bake-off winner, 2026-09-09; design document §9–§10). The mass stays opaque; its own
// vertex colours take the ORGAN's albedo where it meets the organ (the contact rim) and keep the mass colour at the
// apex, so the boundary dissolves without translucency and without any mass hue entering the organ. The band height is
// the cited category's extent (illustrative) through the ONE formula shared with reserve_check (rimBlendWeight); on a
// margin-reserved mass the extent is CAPPED by identity at RESERVED_APEX.capForReservedMargin so the reserved apex keeps
// reading (measured floor), and the cap is disclosed on the badge. The four losing channels (opacity; organ-side albedo
// bleed, roughness, darken) were removed with the wiring — the organ-side family is retired STRUCTURALLY (a mass is 22%
// of the organ radius, so a ring in mass radii is organ-scale); their captures and numbers are in the design document.
const albedoCache = new WeakMap();
function organAlbedo(material){
  // A material with no texture carries its albedo in material.color; a textured scan (lungs, thyroid, stomach, colon)
  // carries it in the texture and its material.color is WHITE — so: colour × the texture's mean, taken once per material.
  ...
  if(img && img.width && img.height){
    try{
      ...
      const n = d.length / 4;
      c.multiply(new THREE.Color(r / n / 255, g / n / 255, b / n / 255).convertSRGBToLinear());
    }catch(e){ /* a cross-origin or not-yet-decoded image: fall back to material.color */ }
  }
  albedoCache.set(material, c);
  return c;
}
function applyRimBlend(viewer, mass, requestedExtent){
  ...
  if(!organColour) return null;
  mass.geometry.computeBoundingSphere();
  const massR = mass.geometry.boundingSphere.radius;
  ...
  for(let i = 0; i < n; i++){
    ...
    const w = rimBlendWeight(hgt, massR, extent);
  ...
  const rec = { channel: 'rimBlend', extent, requested: requestedExtent, capped };
  mass.userData.growthFalloff = rec;
  return rec;
}
...
function initSiteViewer(cancerId){
  ...
  detail.regions.forEach((region, idx)=>{
    ...
    organicSpiculate(geo, {
      ...
      spikeCount:7, spikeLength:0.42, sharpness:11, // PHASE A UNCHARACTERISED-MARGIN DEFAULT (RATIFIED 2026-09-09, .claude/phaseA_mapping.md): when cited categories drive these knobs, an entry whose MARGIN half is a recorded NEGATIVE renders ONE shared reserved form on an ORTHOGONAL axis the cited categories never use (not a point on the spike scale — a midpoint reads as "intermediate margin"), UNREACHABLE from any cited category's parameter range (the check ships with the build), plus a NON-OPTIONAL DOM badge and label; no dashed silhouette (a shape is a claim); no silent upgrade.
    });
